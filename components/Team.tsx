...
export const Team: React.FC = () => {
  ...
  // Função para gerar uma cor consistente baseada no nome (para quando não houver foto)
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3 justify-center">
          {authors.map((author, index) => {
            // Gera uma cor de fundo suave baseada no nome
            const bgGradient = `linear-gradient(135deg, ${stringToColor(author.name)}40, ${stringToColor(author.name)}80)`;
            
            return (
              <div 
                key={index} 
                className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg hover:shadow-xl border border-slate-100 dark:border-slate-700 transition-all duration-300 hover:-translate-y-2 flex flex-col items-center text-center"
              >
                <div className="relative w-28 h-28 mb-5">
                   <div className="w-full h-full rounded-full border-4 border-slate-50 dark:border-slate-700 shadow-md overflow-hidden bg-slate-100 flex-shrink-0 relative z-10">
                    <img 
                      ...
                      onError={(e) => {
                        // Esconde a imagem quebrada e mostra o fallback
                        ...
                        if (parent) {
                          ...
                          // Verifica se já existe o span de iniciais para não duplicar
                          if (!parent.querySelector('.initials-fallback')) {
                            const span = document.createElement('span');
                            span.className = "initials-fallback font-bold text-3xl text-slate-700 dark:text-slate-200 drop-shadow-sm";
                            span.innerText = getInitials(author.name);
                            parent.appendChild(span);
                          }
                        }
                      }}
                    />
                    {/* Fallback inicial (renderizado se o JS não rodar a tempo ou imagem estiver vazia) */}
                    {!author.image && (
                       <div 
